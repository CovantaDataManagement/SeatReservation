...
function App() {
    ...
    const fetchSeats = async () => {
        ...
        setError(null);
        try {
            const availableRes = await fetch(`http://10.99.18.77:5000/api/seats/available?date=${encodeURIComponent(date)}`);
            const reservedRes = await fetch(`http://10.99.18.77:5000/api/seats/reserved?date=${encodeURIComponent(date)}`);
    ...
    const handleReservation = async (e) => {
        ...
        try {
            ...
            const response = await fetch('http://10.99.18.77:5000/api/reservations', {
                method: 'POST',
